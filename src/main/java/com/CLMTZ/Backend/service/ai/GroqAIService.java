...
public class GroqAIService {
     ...
     * A diferencia de chat(), no fuerza formato JSON — ideal para el chatbot conversacional.
     */
    public String chatFreeText(String systemPrompt, String userPrompt) {
        ...
        try {
            ...
            if (choicesNode != null && choicesNode.isArray() && !choicesNode.isEmpty()) {
                JsonNode messageNode = choicesNode.get(0).get("message");
                if (messageNode != null) {
                    log.info("[GroqAIService] chatFreeText: respuesta recibida");
                    return messageNode.get("content").asText();
                }
            }

            log.error("[GroqAIService] chatFreeText: respuesta sin contenido válido");
            return null;

        } catch (HttpClientErrorException e) {
            log.error("[GroqAIService] Error HTTP en chatFreeText: {} - {}", e.getStatusCode(), e.getResponseBodyAsString());
            throw new GroqAIException("Error de API Groq: " + e.getMessage(), e);
        } catch (Exception e) {
            log.error("[GroqAIService] Error en chatFreeText: {}", e.getMessage());
            throw new GroqAIException("Error de comunicación con Groq: " + e.getMessage(), e);
        }
    }
